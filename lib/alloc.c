...
static size_t
REGPARM(1) get_index(size_t _size) {
...
static void
REGPARM(2) __small_free(void* _ptr, size_t _size) {
...
static void*
REGPARM(1) __small_malloc(size_t _size) {
  __alloc_t* ptr;
  size_t size = _size;
  size_t idx;
  ...
  if(ptr) {
    ...
    if(size) {
      if(size <= __MAX_SMALL_SIZE)
        ...
        char* block = BLOCK_START(ptr);
         size_t pad =  ((size_t)block & (PAGE_SIZE - 1));
        munmap(block-pad, size + pad);
       }
    }
  }
}
...
static void*
_alloc_libc_malloc(size_t size) {
  __alloc_t* ptr;
  size_t need;
#ifdef WANT_MALLOC_ZERO
  if(!size)
...
void*
__libc_calloc(size_t nmemb, size_t _size) {
  size_t size;
#if __GNUC__ >= 5
...
void*
__libc_realloc(void* ptr, size_t _size) {
...
int
__libc_posix_memalign(void** memptr, size_t align, size_t size) {
  size_t block_pad;
  void* ptr;
  if(align > MEM_BLOCK_SIZE)
    return -1;

  if(align > sizeof(__alloc_t))
    block_pad = align - sizeof(__alloc_t);
  else
    block_pad = 0;

  if((ptr = _alloc_libc_malloc(size + block_pad))) {
    *memptr = (char*)ptr + block_pad;

    if(block_pad)
      ((size_t*)*memptr)[-1] = ((size_t*)ptr)[-1];
...
int posix_memalign(void** memptr, size_t align, size_t size) __attribute__((weak, alias("__libc_posix_memalign")));
